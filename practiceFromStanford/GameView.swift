...
struct GameView: View {
    ...
    // @ObservedObject дает сигнал менять модельку как только заметит, что что-то изменилось
    ...
    private func isUndeal (_ card: EmojiMemoryGame.Card) -> Bool {!dealt.contains(card.id)} //helper
    ...
    var gameBody: some View {
        AspectVGrid (items: game.cards, aspectRatio: 2/3) { card in
            if isUndeal(card) || card.isMatched && !card.isFaceUp {Color.clear}
            else {
                CardView(card: card)
...
struct CardView: View {
    ...
    var body: some View {
        GeometryReader(content: {geometry in
            ...
            .cardify(isFaceUp: card.isFaceUp) //наш самодельный модификатор
